...
public:
  ...
  FlightRoute(int duration = 0, const std::string &airline = "")
      : duration(duration), airline(airline) {}
  ...
  friend std::ostream &operator<<(std::ostream &os, const FlightRoute &route) {
    os << "{ Duration: " << route.duration << " mins, Airline: \""
       << route.airline << "\" }";
    return os;
  }
};
...
int main() {
  ...
  // flightGraph.getEdgeValue("Paris", "New York"); //throws error, handle
  // carefully
