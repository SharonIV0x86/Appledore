...
void printAdjacencyMatrix(
    const Appledore::MixedGraphMatrix<VertexType, EdgeType> &graph) {
  size_t numVertices = graph.getNumVertices();
  const auto &vertices =
      graph.getVertices(); // Get vertices directly from the graph

  for (size_t i = 0; i < numVertices; ++i) {
    for (size_t j = 0; j < numVertices; ++j) {
      std::cout << (graph.hasEdge(vertices[i], vertices[j])
                        ? std::to_string(
                              graph.getEdgeValue(vertices[i], vertices[j]))
                        : "0")
                << " ";
    }
    std::cout << "\n";
