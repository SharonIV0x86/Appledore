...
void printEdges(const auto &edges, const std::string &description) {
  ...
  for (const auto &edge : edges) {
    std::cout << "Edge from " << std::get<0>(edge) << " to "
              << std::get<1>(edge) << " with weight " << std::get<2>(edge)
              << "\n";
  }
  std::cout << '\n';
}
...
int main() {
  try {
    ...
    std::vector<std::tuple<TestVertex, TestVertex, float>> edges4 =
        graph2.getAllEdges(std::nullopt, [](const auto &a, const auto &b) {
          return std::get<2>(a) >
                 std::get<2>(b); // Custom: Sort by weight in descending order
        });
